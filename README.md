...
### 1. 视频旋转功能
- 支持 0°、90°、180°、270° 四种旋转角度
- 实现像素级视频旋转处理
- 自动将旋转后的视频数据传递给被HOOK的应用程序

### 2. 视频文件选择界面
- 点击"选择视频文件"按钮弹出二级菜单
- 显示 DCIM/Camera 目录下的所有视频文件
- 按修改时间倒序排序，显示文件名、大小、修改日期
- 提供"选择更多文件"按钮浏览其他目录

### 3. 自动复制视频功能
- 支持ROOT权限自动复制视频到目标应用私有目录
- 确保视频文件在不同权限环境下都能正常访问
- 自动处理目录重定向和权限问题
...
1. 安装此模块，并在Xposed中启用此模块，Lsposed等包含作用域的框架需要选择目标app，无需选择系统框架。需要ROOT权限。
...
1. 在主界面选择需要的旋转角度（0°、90°、180°、270°）
2. 选择视频文件（从DCIM/Camera目录或其他位置）
3. 点击"复制视频到应用私有目录"按钮
4. 打开目标应用，系统会自动应用旋转设置

> 注意：预览窗口仅显示视频的正常状态，无法实时展示完整的旋转效果。旋转效果会在被HOOK的应用中体现。
